...
class AgentV2(object):
    ...
    def append_sample(self, state, action, reward, next_state, done):
        target = self.Q_eval(state).data
        old_val = target[action]
        target_val = self.Q_next(next_state).data
        if done:
            target[action] = reward
        else:
            target[action] = reward + self.GAMMA * T.max(target_val)
    ...
    def storeTransition(self, state, action, reward, state_, terminal):
        ...
        self.append_sample(state, action, reward, state_, terminal)
